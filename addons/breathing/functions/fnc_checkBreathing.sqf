...
if (_hr == 0 || !(alive _patient) || ((_patient getVariable [QEGVAR(airway,obstruction), 0]) isEqualTo 1 && !(_patient getVariable [QEGVAR(airway,overstretch), false])) || (_patient getVariable [QEGVAR(airway,obstruction), 0]) isEqualTo 2 || _patient getVariable [QEGVAR(airway,occluded), false] || _patient getVariable [QGVAR(hemopneumothorax), false] || _patient getVariable [QGVAR(tensionpneumothorax), false]) then {
